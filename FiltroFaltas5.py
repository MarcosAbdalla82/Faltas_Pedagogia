...
if arq is not None:
    ...
    SENAI = df['COD_TURMA'].str.split('-', expand=True)
    SENAI = SENAI[5].unique()
    ESC = st.selectbox('Escola - Centro de Custo',options=SENAI,index = 2)
    
    
    turmasSAT = df[df["COD_TURMA"].str.contains(ESC, na=False)]#13490
